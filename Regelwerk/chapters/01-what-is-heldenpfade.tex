...
\begin{kurzregel}[title={Grundsatz: Die Spielleitung entscheidet}]
Die Regeln bilden den gemeinsamen Rahmen für das Spiel. Lassen sie eine Situation bewusst oder unbewusst offen, entscheidet die Spielleitung, wie sie ausgelegt wird.

Dabei sollte sie stets eine Lösung wählen, die zur Situation passt und den gemeinsamen Spielspaß fördert.

Spielgruppen können Regeln jederzeit gemeinsam an ihre eigenen Vorlieben anpassen.
\end{kurzregel}
...
\textbf{Spielleitung:} „Dann führe bitte eine Suchen-Probe durch.“
\end{quote}
...
Die Beispielhelden befinden sich in \appendixref{app:example-heroes}.

Wenn ihr lieber einen eigenen Helden erschaffen möchtet, findet ihr die vollständigen Regeln dazu in \chapterref{chap:hero-creation}.
...
\section{Kartenzustände}

Jede Karte besitzt eine Vorder- und eine Rückseite. Die Vorderseite zeigt die Karte im \textbf{bereiten} Zustand. Auf der Rückseite ist ihre Illustration zur besseren Erkennbarkeit in Schwarz-Weiß dargestellt; die Karte ist dann \textbf{erschöpft}.

Das Symbol \exhaust{} bedeutet immer, die Karte umzudrehen. Wird eine bereite Karte von ihrer Vorderseite auf die Rückseite gedreht, wird sie erschöpft. Wird eine erschöpfte Karte von ihrer Rückseite auf die Vorderseite gedreht, wird sie bereit gemacht.

Es gelten ausschließlich die Angaben, Werte und Effekte der aktuell sichtbaren Kartenseite. Wie und wann eine erschöpfte Karte wieder bereit gemacht werden kann, ergibt sich aus ihrer Rückseite oder aus anderen Regeln.

\begin{kurzregel}[title=Bereite und erschöpfte Karten]
\begin{itemize}
  \item \textbf{Bereit:} Die Vorderseite der Karte liegt oben.
  \item \textbf{Erschöpft:} Die schwarz-weiße Rückseite der Karte liegt oben.
  \item \exhaust{}: Drehe die Karte um.
\end{itemize}
\end{kurzregel}
